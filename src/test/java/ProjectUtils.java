 ...
 * Mac
 * 第一步
 * gsed -i "s/huomai/doctor/g" `grep huomai -rl /Users/chenshufeng/doctor-plat`
 * 第二步
 * find /Users/chenshufeng/doctor-plat -name huomai | sed ''s/huomai/mv huomai doctor/'' | sh
 * 第三步
 * 重命名文件名称
 * @author chenshufeng
 * @version 1.0
...
public class ProjectUtils {
    ...
    public static void allRename(File[] files) {
        ...
        Arrays.stream(files).filter(file -> file.getName().contains(old_str)).collect(Collectors.toList()).forEach(file -> FileUtil.rename(file, file.getName().replace(old_str, new_str), true, true));
    }
}
